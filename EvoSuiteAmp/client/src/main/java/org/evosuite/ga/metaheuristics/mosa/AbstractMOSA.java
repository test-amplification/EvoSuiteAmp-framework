...
public abstract class AbstractMOSA extends GeneticAlgorithm<TestChromosome> {
    ...
    protected List<TestChromosome> breedNextGeneration() {
        ...
        for (int i = 0; i < Properties.POPULATION / 2 && !this.isFinished(); i++) {
            ...
            String name1 = parent1.test.getName();
            String name2 = parent2.test.getName();
            TestChromosome offspring1 = parent1.clone();
            TestChromosome offspring2 = parent2.clone();

            offspring1.test.setName(name1+name2);
            offspring2.test.setName(name2+name1);
            // apply crossover
...
//         }
    ...
    // protected List<TestChromosome> breedNextGeneration() {
    //     List<muDistanceAnalysis> muDistancePopulation = this.muDistanceAnalysisPopulation;
    //     List<TestChromosome> offspringPopulation = new ArrayList<>(Properties.POPULATION);
    //     // we apply only Properties.POPULATION/2 iterations since in each generation
    //     // we generate two offsprings
    //     for (int i = 0; i < Properties.POPULATION && !this.isFinished(); i++) {
    //         // select best individuals
    //         TestChromosome parent1 = this.selectionFunction.select(this.population);
    //         String name = parent1.test.getName();
    //         if (name != null) {
    //             List<muDistanceAnalysis> selectedPopulation = muDistancePopulation.stream().filter(muDistance -> muDistance.getTestName().contains(name)).collect(Collectors.toList());
    //         }
    //         TestChromosome offspring1 = parent1.clone();
    //         offspring1.test.setName(parent1.test.getName());
    //         // this.removeUnusedVariables(offspring1);
    //         // int index = this.muDistanceAnalysisPopulation.indexOf(mDistance);
    //         // mDistance.setDistance(tCMDistance);
    //         // mDistance.setTestChromosome(tC);
    //         // this.muDistanceAnalysisPopulation.set(index, mDistance);

    //         // apply mutation on offspring1
    //         offspring1.mutChange();
    //         if (offspring1.isChanged()) {
    //             this.clearCachedResults(offspring1);
    //             offspring1.updateAge(this.currentIteration);
    //             this.calculateFitness(offspring1);
    ...
    //     // List<TestChromosome> offspringPopulation = new ArrayList<>(Properties.POPULATION);
    //     List<TestChromosome> offspringPopulation = new ArrayList<>(muDistancePopulation.size());
    //     // we apply only Properties.POPULATION/2 iterations since in each generation
    //     // we generate two offsprings  
    ...
    //         && (distance.getValue() <= tCMDistance.getValue() && distance.getKey().equals(tCMDistance.getKey())) 
    //         || (distance.getKey().equals("iDM") && (tCMDistance.getKey().equals("eDM") || tCMDistance.getKey().equals("One")))
    //         || (distance.getKey().equals("eDM") && (tCMDistance.getKey().equals("One")))) {
    //         // while (tCMDistance.getValue() > 0){
    //             int changeTestChromosome = 0;
    //             while(!tC.isChanged()){
    //                 if(tCMDistance.getKey().equals("eDM")){
    //                     // tC.mutInsert();
    //                     // if(changeTestChromosome > 2){
    //                     //     tC.mutDelete();
    //                     // }
    //                     tC.mutChange();
    //                 } else if(tCMDistance.getKey().equals("iDM")) {
    //                     tC.mutChange();
    //                 } else if(tCMDistance.getKey().equals("One")) {
    //                     tC.mutChange();
    //                     // tC.mutDelete();
    //                 }
    //                 changeTestChromosome++;
    //             }

    //             this.removeUnusedVariables(tC);
    ...
    //                     tC.setChanged(false);
    //                 }
    //                 String testName = mDistance.getTestName() + "__" + mDistance.getGoal();
    //                 tCMDistance = tCF.getFitness2(tC, result, testName);
    //                 // logger.warn("Distance" + distance);
                    
    //                 if ((tCMDistance.getValue() == 0
    //                 || (tCMDistance.getValue() < distance.getValue() && distance.getKey().equals(tCMDistance.getKey()))
    //                 || (distance.getKey().equals("eDM") && tCMDistance.getKey().equals("iDM")))) {
    //                 // if (tCMDistance.getKey().equals("")) {
    //                     logger.warn("got em");
    //                     tC.updateAge(this.currentIteration);
    //                     this.calculateFitness(tC);
    //                     offspringPopulation.add(tC);
    //                     int index = this.muDistanceAnalysisPopulation.indexOf(mDistance);
    //                     mDistance.setDistance(tCMDistance);
    //                     mDistance.setTestChromosome(tC);
    //                     this.muDistanceAnalysisPopulation.set(index, mDistance);

...
public abstract class TestFitnessFunction
    ...
    public boolean isCovered(TestChromosome tc, String flag) {
        if (tc.getTestCase().isGoalCovered(this)) {
            return true;
        }
    ...
    public boolean isCovered(TestChromosome individual, ExecutionResult result, String flag) {
        boolean covered = getFitness(individual, result, flag) == 0.0;
        if (covered) {
            individual.test.addCoveredGoal(this);
        }
        return covered;

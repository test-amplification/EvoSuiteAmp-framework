...
public abstract class PrimitiveStatement<T> extends AbstractStatement {
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    public static PrimitiveStatement<?> getPrimitiveStatement2(TestCase tc,
                                                              Class<? extends CtStatement> genericClass, CtStatement ctStatement) {
        // TODO This kills the benefit of inheritance.
        // Let each class implement the clone method instead

        Class<?> clazz = genericClass;
        PrimitiveStatement<?> statement;

        if (clazz == boolean.class) {
            statement = new BooleanPrimitiveStatement(tc);
        } else if (clazz == CtLocalVariableImpl.class){
            statement = new EmptyPrimitiveStatement(tc);
        } else if (clazz == CtInvocationImpl.class){
            statement = new EmptyPrimitiveStatement(tc);
        } else if (clazz == CtAssignmentImpl.class){
            statement = new EmptyPrimitiveStatement(tc);
        } else if (clazz == int.class) {
            statement = new IntPrimitiveStatement(tc);
        } else if (clazz == char.class) {

    ...
    private void readTestCases() throws IllegalStateException {
        ...
        for (Class<?> targetClass : testMap.keySet()) {
            ...
            for (TestCase test : testMap.get(targetClass)) {
                ...
                } else if(executionResult.hasUndeclaredException() || executionResult.hasTestException() || executionResult.hasSecurityException()) {
                    logger.warn("Adding carved test with exception");
                    logger.info(test.toCode());
                    processedTests.add(test);
                }else {

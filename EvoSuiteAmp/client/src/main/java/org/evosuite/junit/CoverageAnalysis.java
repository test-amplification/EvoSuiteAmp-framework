...
public class CoverageAnalysis {
     ...
     * @return
     */
    public static List<muDistanceAnalysis> getCoveredGoals2(Class<?> testClass, List<TestFitnessFunction> allGoals, String[] testClasses) {
        ...
        List<muDistanceAnalysis> coveredGoals = new ArrayList<muDistanceAnalysis>();
        CarvingManager manager = CarvingManager.getInstance();
        final Class<?> targetClass = Properties.getTargetClassAndDontInitialise();
        List<org.evosuite.testcase.TestCase> testCases = manager.getTestsForClass(targetClass);
        List<MutablePair<org.evosuite.testcase.TestCase,Boolean>> testBoolean = new ArrayList<MutablePair<org.evosuite.testcase.TestCase, Boolean>>();
        for(org.evosuite.testcase.TestCase testCase : testCases){
            testBoolean.add(new MutablePair<org.evosuite.testcase.TestCase,Boolean> (testCase, false));
        }

        // Launcher launcher = new Launcher();
        // launcher.addInputResource("/Usersaa/firhardroslan/Desktop/test-amplification/defects4j/Lang1f/src/test");
        ...
        // TestRunner testRunner = new TestRunner("/Usersaa/firhardroslan/Desktop/test-amplification/defects4j/Lang1f/src/test", "", false);
        // CtClass<Object> testClass1 = launcher.getFactory().Class().get(testClass);
        // List<org.evosuite.testcase.TestCase> tests1234 = manager.getTestsForClass(testClass1);
        
        // analyzeCoverage();
        List<JUnitResult> results = executeTests(testClass);
        // List<JUnitResult> results = executeTests2(testClass1); //running with DSpot 
        for (JUnitResult testResult : results) {
            ...
            // CtMethod<?> originalMethod = testClass1.getMethods().stream().filter(m -> testResult.getName().split("#")[1].equals(m.getSimpleName())).findFirst().get();

            for (TestFitnessFunction goal : allGoals) {
                String testName = testResult.getName() + "__" + goal;
                if (!goal.isCovered(dummy, testName)) {
                    ExecutionResult result = dummy.getLastExecutionResult();
                    TestChromosome tC = new TestChromosome();
                    for (MutablePair <org.evosuite.testcase.TestCase,Boolean> test : testBoolean){
                        if(testResult.getName().contains(test.getKey().getName())) {
                            // List<CtStatement> statements = originalMethod.getBody().getStatements();
                            // for (CtStatement statement : statements){
                            //     if(test.getValue() != true){
                            //         PrimitiveStatement primitiveValue = PrimitiveStatement.getPrimitiveStatement2(test.getKey(), statement.getClass(), statement);
                            //         primitiveValue.setValue(statement.toString());
                            //         test.getKey().addStatement(primitiveValue);
                            //     }
                            // }
                            test.setValue(true);
                            tC.setTestCase(test.getKey());
                            break;
                        }
                    }
                    // logger.warn(originalMethod.getBody() + ": originalMethod.getBody()");
                    // result.setExecutedStatements(1);
                    // tC2.setTestCase(executionResult.test);
                    muDistanceAnalysis objt = new muDistanceAnalysis(goal, goal.distance(dummy, testName), testResult.getName(), tC);
                    coveredGoals.add(objt);
                }
            }
        }
    ...
    public static Set<TestFitnessFunction> getCoveredGoals(Class<?> testClass, List<TestFitnessFunction> allGoals, String[] testClasses) {
        ...
        for (JUnitResult testResult : results) {
            ...
            for (TestFitnessFunction goal : allGoals) {
                if (goal.isCovered(dummy, testResult.getName() + "__" + goal)) {
    ...
    private static List<JUnitResult> executeTests2(CtClass<Object> testClasses) {

        ExecutionTracer.enable();
        ExecutionTracer.setCheckCallerThread(false);
        ExecutionTracer.getExecutionTracer().clear();
        Set<CtMethod<?>> ctMethods = testClasses.getAllMethods();

        List<JUnitResult> results = new ArrayList<>();
        for (CtMethod<?> ctMethod : ctMethods) {
            LoggingUtils.getEvoLogger().info("  Executing " + ctMethod.getSimpleName());
            // Set the context classloader in case the SUT requests it
            Thread.currentThread().setContextClassLoader(ctMethod.getClass().getClassLoader());
            JUnitRunner jR = new JUnitRunner(ctMethod.getBody().getClass());
            jR.run();
            results.addAll(jR.getTestResults());
        }

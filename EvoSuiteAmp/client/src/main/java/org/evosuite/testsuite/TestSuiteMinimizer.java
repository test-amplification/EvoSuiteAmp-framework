...
public class TestSuiteMinimizer {
    ...
    public void filterJUnitCoveredGoals(List<TestFitnessFunction> goals) {
        ...
        for (String testClass : testClasses) {
            try {
                ...
                Set<TestFitnessFunction> coveredGoals = CoverageAnalysis.getCoveredGoals(junitClass, goals, testClasses);
                // LoggingUtils.getEvoLogger().info("* Removing " + coveredGoals.size() + " goals " + coveredGoals.toString() + " already covered by JUnit (total: " + goals.size() + ")");
    ...
    public List<muDistanceAnalysis> filterJUnitCoveredGoals2(List<TestFitnessFunction> goals) {
        Set<TestFitnessFunction> coveredGoals = null;
        List<muDistanceAnalysis> coveredGoals2 = null;
        if (Properties.JUNIT.isEmpty())
            return null;
        ...
        for (String testClass : testClasses) {
            try {
                Class<?> junitClass = Class.forName(testClass, true, TestGenerationContext.getInstance().getClassLoaderForSUT());
                coveredGoals2 = CoverageAnalysis.getCoveredGoals2(junitClass, goals, testClasses);
                coveredGoals = CoverageAnalysis.getCoveredGoals(junitClass, goals, testClasses);
                // LoggingUtils.getEvoLogger().info("* Removing " + coveredGoals.size() + " goals " + coveredGoals.toString() + " already covered by JUnit (total: " + goals.size() + ")");
                //logger.warn("Removing " + coveredGoals + " goals already covered by JUnit (total: " + goals + ")");
                goals.removeAll(coveredGoals);
                logger.info("Remaining goals: " + goals.size() + ": " + goals);
